...
ATTRIB_REGEX = re.compile(NODE_ATTRIBUTE_RE)
NODE_REGEX = re.compile(NODE_RE)
...
def split_path(path: str) -> list[tuple[str, str, str, str]]:
    ...
    """
    return NODE_REGEX.findall(path)
...
def parse_node_path(path: str) -> tuple[str, dict[str, str]]:
    ...
    path = path.lstrip("./")
    for attribute in ATTRIB_REGEX.findall(path):
...
class Node:
    ...
    def __init__(self, name: str, attributes: dict[str, str] = {}, text: str | None = None) -> None:
        """Init for Node class.
